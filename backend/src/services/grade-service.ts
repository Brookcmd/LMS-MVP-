...
export async function studentGrades(userIdValue: string, schoolIdValue: string, academicYear: unknown, q: unknown) { const userId = id(userIdValue, "userId"); const schoolId = id(schoolIdValue, "schoolId"); const student = await prisma.student.findFirst({ where: { userId, schoolId } }); if (!student) throw appErrors.notFound("Student record not linked to this user account"); const report = await classResults(String(student.classId), String(schoolId), academicYear, q); return { ...report, result: report.results.find((row) => row.student.id === student.id) }; }
